var $haz = jQuery.noConflict();
(function( $haz ) {
	'use strict';

        $haz('.wcvaattributecolorselect').wpColorPicker();
        ...
        jQuery(document).on( 'change', '#tag-name', function( event ){
				    
			var termname = $haz(this).val();

			$haz('#wcvatextblockp').find('.wcvatextblockinput').val(termname);

			return false;
		});


		jQuery(document).on( 'change', '#name', function( event ){
        ...
        $haz(".image-upload-div").each(function(){
    	    var parentId = $haz(this).closest('div').attr('idval');
		 		 // Only show the "remove image" button when needed
		    var srcvalue    = $haz('#thumbnail_id_' + parentId + '').val();
